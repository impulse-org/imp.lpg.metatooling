...
public class JikesPGView extends ViewPart {
	...
	static void openViewDirect() throws PartInitException {
		...
		try { page.showView("org.jikespg.uide.views.JikesPGView"); } catch (Exception e) { }
		try { page.showView("org.eclipse.ui.views.TaskView"); } catch (Exception e) { }
		try { page.showView("org.eclipse.ui.views.ProblemView"); } catch (Exception e) { }
	}
		...
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				try {
					...
					if (singleton.text.isDisposed()) openViewDirect();

...
public class DiagnoseParser implements ParseErrorCodes
{
    ...
    private int maxErrorsToReport = Integer.MAX_VALUE;
    ...
    private static final int LARGEST_REASONABLE_STACK = 100;
    ...
    public void diagnose()
    {
        ...
        //
        if (error_token != 0)
            diagnose(error_token, Integer.MAX_VALUE);

        return;
    }

    public void diagnose(int error_token, int number_errors_to_report)
    {
    	maxErrorsToReport = number_errors_to_report;
        ...
        } while (act != ACCEPT_ACTION && maxErrorsToReport > 0);
    ...
    int parseForError(int current_kind)
    {
        ...
        //
        for (int dizzy=0; dizzy<100; dizzy++)
        {
            if (act <= NUM_RULES)
            {
                tempStackTop--;
            ...
            if (tempStackTop > LARGEST_REASONABLE_STACK)
                break;
        }
    ...
    private RepairCandidate errorRecovery(int error_token)
    {
        ...
        // tokens.
        //
        while(tokStream.getKind(buffer[BUFF_UBOUND]) != EOFT_SYMBOL && maxErrorsToReport > 0)
        {
                             ...
                             int scope_name_index)
    {
        ...
        String token_name = (name_index >= 0 && 
        		!name(name_index).equalsIgnoreCase("ERROR") ? "\"" + name(name_index) + "\"" : "");
